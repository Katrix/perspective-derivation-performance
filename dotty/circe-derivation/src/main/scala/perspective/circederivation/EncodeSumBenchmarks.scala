...
class EncodeSumBenchmarks {
  ...
  @Benchmark
  def encodeShapeless3(state: Shapeless3SumDefs.EncodeData): Json =
    Shapeless3SumDefs.encode(state.size, state.getData)
  ...
  @Benchmark
  def encodePerspectiveInline(state: PerspectiveInlineSumDefs.EncodeData): Json =
    PerspectiveInlineSumDefs.encode(state.size, state.getData)
}
